    ...
    if (empty($name) || empty($email) || empty($password) || empty($role)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                // For LAWYER role, verification file is REQUIRED
                $verificationFile = null;
                $verificationFilename = null;

                if ($role === 'LAWYER') {
                    if (!isset($_FILES['verification_file']) || $_FILES['verification_file']['error'] !== UPLOAD_ERR_OK) {
                        $error = 'Verification document is required for lawyer registration';
                    }
                }

                // Handle file upload if provided (LAWYER required; OFFICIAL optional)
                if (!$error && ($role === 'LAWYER' || $role === 'OFFICIAL')) {
                    if (isset($_FILES['verification_file']) && $_FILES['verification_file']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!$error) {
                    ...
                    if (!$error) {
                        ...
                        $stmt = $pdo->prepare('INSERT INTO users (email, name, role, district, password_hash, verified, verification_file_path) VALUES (:email, :name, :role, :district, :password_hash, 0, :verification_file_path)');
                        $stmt->execute([
                            'email' => $email,
                            'name' => $name,
                            'role' => $role,
                            ...
                            'verification_file_path' => $verificationFile
                        ]);
